...
export async function sendVerificationEmail({ to, verifyUrl, appName = 'COMET' }) {
  ...
  try {
    ...
    console.log('[email-debug] Intentando Gmail Service Account...');
    const saAttempt = await sendWithGmailServiceAccount({ to, subject, html });
    console.log('[email-debug] Gmail SA result:', saAttempt);
    if (saAttempt.ok) {
    ...
    console.log('[email-debug] Intentando Gmail OAuth...');
    const gmailAttempt = await sendWithGmail({ to, subject, html });
    console.log('[email-debug] Gmail OAuth result:', gmailAttempt);
    if (gmailAttempt.ok) {
    ...
    if (apiKey) {
      console.log('[email-debug] Intentando Resend...');
      ...
      console.log('[email-debug] Resend response status:', r.status);
      ...
      console.log('[email-debug] Intentando Webhook...');
      const r = await axios.post(hook, { to, subject, html }, { timeout: 10000, validateStatus: () => true });
      console.log('[email-debug] Webhook response status:', r.status);
      if (r.status >= 200 && r.status < 300) {
    ...
    // Instead of silently failing, throw the error so controller can handle it
    throw new Error(`Email send failed: ${data?.message || String(e)}`);
  }
  console.log(`[email] ⚠️ MODO MOCK - Email a ${to}: ${verifyUrl}`);
  console.error('[email] ✗✗✗ CRITICAL: No email provider configured! Email NOT sent.');
  // Throw error in production so we know emails aren't being sent
  if (process.env.NODE_ENV === 'production') {
    throw new Error('No email provider configured - email not sent');
  }
  return { ok: true, mocked: true };
}

// Send a password reset email; does not reveal account existence
export async function sendPasswordResetEmail({ to, resetUrl, appName = 'COMET' }) {
  ...
  const html = `
    <div style="font-family:system-ui,Segoe UI,Roboto,Helvetica,Arial,sans-serif;max-width:560px;margin:auto;padding:16px">
      ...
      <p style="margin:24px 0;text-align:center">
        <a href="${resetUrl}" style="background:#1e40af;color:#fff;padding:12px 18px;border-radius:8px;text-decoration:none;display:inline-block" target="_blank" rel="noreferrer">Restablecer contraseña</a>
      </p>
      <p>Si no solicitaste esto, puedes ignorar este correo.</p>
      <p>O copia y pega este enlace en tu navegador:</p>
      <p><a href="${resetUrl}">${resetUrl}</a></p>
      <p style="color:#6b7280;font-size:12px">Este enlace expira en 1 hora.</p>
    </div>
  `;
  ...
  try {
    ...
    if (apiKey) {
      const from = process.env.MAIL_FROM || 'noreply@comet.local';
      const r = await axios.post('https://api.resend.com/emails', { from, to: [to], subject, html }, {
        headers: { Authorization: `Bearer ${apiKey}` }, timeout: 10000, validateStatus: () => true,
      });
      if (r.status >= 200 && r.status < 300) return { ok: true };
      const msg = r.data?.message || r.data?.error || r.statusText;
      console.warn('[reset] Resend rejected message:', r.status, msg);
    } else if (hook) {
      const r = await axios.post(hook, { to, subject, html }, { timeout: 10000, validateStatus: () => true });
      if (r.status >= 200 && r.status < 300) return { ok: true };
      console.warn('[reset] Mail webhook rejected message:', r.status, r.data?.message || r.statusText);
    }
  } catch (e) {
    const status = e?.response?.status;
    const data = e?.response?.data;
    console.warn('[reset] Email send error:', status, data?.message || String(e));
  }
  console.log(`[reset] Send email to ${to}: ${resetUrl}`);
  return { ok: true, mocked: true };
}
